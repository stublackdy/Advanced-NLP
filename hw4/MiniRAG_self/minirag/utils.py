...
def count_elements_in_tuple(tuple_elements, list_elements):
    ...
    # Count how many elements in tuple_elements also appear in list_elements
    return sum(elem in list_set for elem in tuple_elements)
...
def cal_path_score_list(candidate_reasoning_path, maybe_answer_list):
    ...
    for k, v in tqdm(candidate_reasoning_path.items()):
        score = v["Score"]
        paths = v["Path"]

        # Use dictionary comprehension for compactness and speed
        scores = {p: [sum(elem in maybe_answer_set for elem in p)] for p in paths}
...
#     for i in tqdm(return_dict.items()):
#         for j in tqdm(i[1]["Path"].items()):
#             if j[1]:
#                 count = 0
...
def edge_vote_path(path_dict, edge_list):
    # Shallow copy is usually sufficient unless path_dict values are mutated
    return_dict = copy.deepcopy(path_dict)

    # Precompute all (src_id, tgt_id) pairs once
    edge_pairs = [(e["src_id"], e["tgt_id"]) for e in edge_list]

    # Prepare cache to avoid recomputing is_continuous_subsequence
    subseq_cache = {}

    pairs_append = {}

    # Outer loop with progress bar only once
    for key, value in tqdm(return_dict.items(), desc="Processing paths"):
        for path, scores in value["Path"].items():
            if not scores:
                continue
            ...
            for pair in edge_pairs:
                cache_key = (pair, path)
                if cache_key in subseq_cache:
                    match = subseq_cache[cache_key]
                else:
                    match = is_continuous_subsequence(pair, path)
                    subseq_cache[cache_key] = match
            ...
            if matched_pairs:
                pairs_append[path] = matched_pairs
            scores.append(count)
